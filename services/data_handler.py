...
def update_subscription_stock():
    ...
    if stock_data is None:
        ...
        data_to_insert.extend([{'symbol': symbol, 'category': 'dividend'} for symbol in [item.pdno for item in stock_data]])
    else:  # 단일 객체
        data_to_insert.extend([{'symbol': stock_data.pdno, 'category': 'dividend'}])
