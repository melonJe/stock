...
def korea_investment_trading_buy(ki_api: KoreaInvestmentAPI, symbol: str, price: int, volume: int):
    ...
    dnca_tot_amt = min(inquire_balance["dnca_tot_amt"], inquire_balance["nxdy_excc_amt"], inquire_balance["prvs_rcdl_excc_amt"]) - inquire_balance["tot_evlu_amt"] * 0.10  # 사용 가능한 금액 계산 (예수금총금액 - 총평가금액의 10%)
    try:
        stock = ki_api.get_owned_stock_info(symbol)
        if volume > 0 and stock:  # 구매 수량이 0보다 크고 보유 중인 주식일 경우
            volume = min(volume, int((inquire_balance["tot_evlu_amt"] * 0.2 - stock["pchs_amt"]) / price))  # 주식 보유 비중이 15%를 넘지 않도록 구매 수량 수정
        else:
            volume = min(volume, int(inquire_balance["tot_evlu_amt"] * 0.2 / price))  # 주식 보유 비중이 15%를 넘지 않도록 구매 수량 수정
        if volume > 0 and volume * price < dnca_tot_amt:
...
def korea_investment_trading_sell(ki_api: KoreaInvestmentAPI, symbol: str, price: int, volume: int):
    ...
    try:
        ...
        if (not stock) or stock["ord_psbl_qty"] == 0:  # 가지고 있지 않거나 주문 가능한 수량이 없으면 다음 주식으로 넘어감
            ...
            return
        if volume > stock["ord_psbl_qty"]:  # 주문 가능 수량을 넘길 경우 주문 수량 수정
