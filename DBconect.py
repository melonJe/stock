...
class DBconect:
    ...
    def stock_data_update_daily_naver(self):
        with self.conn.cursor() as cursor:
            ...
            for x in stocks:
                ...
                data = ps.DataFrame(cursor.fetchall(),
                                    columns=('srtnCd', 'basDt', 'hipr', 'clpr', 'lopr', 'trqu')).sort_values(by='basDt')
                url = 'https://finance.naver.com/item/sise.naver?code=' + x
                # print(url)
                html = requests.get(url).text
                soup = BeautifulSoup(html, 'html.parser')
                tags = soup.findAll('dd')
                data = ps.concat([data, ps.DataFrame([[tags[2].text.split(' ')[1],
                                                       datetime.strptime(
                                                           soup.findAll('em', attrs={'class': 'date'})[0].text.split(
                                                               ' ')[0], '%Y.%m.%d').strftime('%Y-%m-%d'),
                                                       tags[6].text.split(' ')[1].replace(',', ''),
                                                       tags[3].text.split(' ')[1].replace(',', ''),
                                                       tags[8].text.split(' ')[1].replace(',', ''),
                                                       tags[10].text.split(' ')[1].replace(',', '')]],
                                                     columns=['srtnCd', 'basDt', 'hipr', 'clpr', 'lopr',
                                                              'trqu']).astype({'clpr': 'int64', 'trqu': 'int64'})])
                ...
                data['PMF'] = 0
                data['NMF'] = 0
                data['perb'] = (data['clpr'] - data['clpr'].rolling(window=20).mean() + 2 * data['clpr'].rolling(
                    window=20).std()) / (
                                       4 * data['clpr'].rolling(window=20).std())
                ...
                data['MFI'] = 100 - (
                        100 / (1 + data['PMF'].rolling(window=10).std() / data['NMF'].rolling(window=10).std()))
                ...
                data = data[
                    ['srtnCd', 'basDt', 'hipr', 'clpr', 'lopr', 'trqu', 'ema5', 'ema20', 'ema60', 'ema120', 'ema240',
                     'PMF', 'NMF', 'perb', 'MFI', 'slow_d']]
        ...
        with self.conn.cursor() as cursor:
            ...
            for x in stocks:
                ...
                data = ps.DataFrame(cursor.fetchall(),
                                    columns=('srtnCd', 'basDt', 'hipr', 'clpr', 'lopr', 'trqu')).sort_values(by='basDt')
                le = list(
                    get_stock_price(beginBasDt=last_date.strftime("%Y%m%d"), endBasDt=date.today().strftime("%Y%m%d"),
                                    likeSrtnCd=x))
                if not le:
                    cursor.execute(delete_sql, x)
                    self.conn.commit()
                    continue
                data = \
                    ps.concat(
                        [data, ps.DataFrame(le).astype({'clpr': 'int64', 'trqu': 'int64'}).sort_values(by='basDt')])[
                        ['srtnCd', 'basDt', 'hipr', 'clpr', 'lopr', 'trqu']]
                data['ema5'] = data['clpr'].ewm(span=5, min_periods=5).mean()
                data['ema20'] = data['clpr'].ewm(span=20, min_periods=20).mean()
                data['ema60'] = data['clpr'].ewm(span=60, min_periods=60).mean()
                data['ema120'] = data['clpr'].ewm(span=120, min_periods=120).mean()
    ...
    def stock_data_update_history(self, stocks):
        with self.conn.cursor() as cursor:
            ...
            for x in stocks:
                data = ps.DataFrame(get_stock_price(beginBasDt=(date.today() - timedelta(days=1000)).strftime("%Y%m%d"),
                                                    endBasDt=date.today().strftime("%Y%m%d"), numOfRows=1000,
                                                    likeSrtnCd=x)) \
                    .astype({'clpr': 'int64', 'trqu': 'int64'})
                ...
                data['slow_d'] = data['fast_k'].rolling(window=3).mean()
                if not data.empty:
                    data = data[['srtnCd', 'basDt', 'hipr', 'clpr', 'lopr', 'trqu', 'ema5', 'ema20', 'ema60', 'ema120',
                                 'ema240', 'PMF', 'NMF', 'perb', 'MFI', 'slow_d']]
    ...
    def bollinger(self):
        ...
        with self.conn.cursor() as cursor:
            ...
            for x in stocks:
                cursor.execute(select_sql, x)
                data = ps.DataFrame(cursor.fetchall(),
                                    columns=['srtnCd', 'basDt', 'hipr', 'clpr', 'lopr', 'trqu', 'perb', 'MFI', 'itmsNm',
                                             'buy'])
    ...
    def triple_screen(self):
        ...
        with self.conn.cursor() as cursor:
            ...
            for x in stocks:
                cursor.execute(select_sql, x)
                data = ps.DataFrame(cursor.fetchall(),
                                    columns=['srtnCd', 'basDt', 'hipr', 'clpr', 'lopr', 'trqu', 'ema120', 'slow_d',
                                             'itmsNm', 'buy']).sort_values(by='basDt')
                if data['buy'].values[-1] == 0:
    ...
    def granville(self):
        ...
        with self.conn.cursor() as cursor:
            ...
            for x in stocks:
                cursor.execute(select_sql, x)
                data = ps.DataFrame(cursor.fetchall(),
                                    columns=['srtnCd', 'basDt', 'hipr', 'clpr', 'lopr', 'trqu', 'ema120', 'itmsNm',
                                             'buy']).sort_values(by='basDt')
                        ...
                        for i in range(-2, -30, -1):
                            if -21 < i:
                                ...
                                if data['ema120'].values[i] < data['lopr'].values[i]:
                                    if data['lopr'].values[i] < data['hipr'].values[-1] and data['ema120'].values[i] < \
                                            data['ema120'].values[-1]:
                                        buy.add(data['itmsNm'].values[-1])
                                    break
                # elif data['buy'].values[-1] == 1:
                #     for i in range(1, len(data)):
